...
#     def sqSum(self):
#         square = self.x*2 + self.y2 + self.z*2
#         return square
...
class user:
    def _init_(self, title, balance, accountno):
        self.title = title
        self.balance = balance
...
class SavingsAccount(user):
    def _init_(self, title, balance, accountno, interestRate):
          super()._init_(title, balance, accountno)
          self.interestRate = interestRate
